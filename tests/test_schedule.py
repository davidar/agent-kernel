...
class TestSchedule:
    ...
    def test_no_pending_wakes(self, tmp_path):
        self._setup(tmp_path)
        assert get_pending_wakes() == []

    def test_pending_wake_due(self, tmp_path):
        self._setup(tmp_path)
    ...
    def test_pending_wake_future(self, tmp_path):
        self._setup(tmp_path)
        future = (datetime.now() + timedelta(hours=1)).isoformat()
        _save_schedule({"wakes": [{"time": future, "reason": "later"}]})
        assert get_pending_wakes() == []

    def test_mark_wake_fulfilled(self, tmp_path):
        self._setup(tmp_path)
    ...
    def test_fulfilled_wake_not_pending(self, tmp_path):
        self._setup(tmp_path)
        past = (datetime.now() - timedelta(minutes=5)).isoformat()
        _save_schedule(
    ...
    def test_cleanup_old_wakes(self, tmp_path):
        self._setup(tmp_path)
